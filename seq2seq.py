...
def beam_decoder(in_seq, in_seq_len, target_seq, target_seq_len, vocab_size,
                 num_units, layers, dropout, beam_width):
    ...
    tiled_seq_len = tf.contrib.seq2seq.tile_batch(in_seq_len, multiplier=beam_width)
    tiled_encoder_output = tf.contrib.seq2seq.tile_batch(encoder_output, multiplier=beam_width)
    decoder_cell = attention_decoder_cell(tiled_encoder_output, tiled_seq_len, num_units,
                                          layers, input_keep_prob)
...
def seq2seq(in_seq, in_seq_len, target_seq, target_seq_len, vocab_size,
            num_units, layers, dropout, beam_width=None):
    in_shape = tf.shape(in_seq)
    batch_size = in_shape[0]
